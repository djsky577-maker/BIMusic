...
(function() {
    function injectStyle() {
        if (document.getElementById('recentHistoryStyle')) return;
        var style = document.createElement('style');
        style.id = 'recentHistoryStyle';
        style.textContent = `
            /* Floating "See All" button on home screen */
            #seeAllRecentBtn {
                display: none;
                position: fixed;
                bottom: 90px;
            ...
            #seeAllRecentBtn.show { display: flex; }
            #seeAllRecentBtn svg {
                width: 14px; height: 14px; fill: #00e0d0;
            }
            #seeAllRecentBtn:active { transform: scale(0.95); }
            ...
            #recentHistoryHeader {
                ...
                align-items: center;
                gap: 12px;
                border-bottom: 1px solid rgba(0, 224, 208, 0.2);
                background: rgba(0, 0, 0, 0.4);
            }
            #recentHistoryBackBtn {
            ...
            #recentHistoryBackBtn:active { background: rgba(0, 224, 208, 0.3); }
            #recentHistoryTitle {
                color: #fff;
                font-size: 20px;
                font-weight: 900;
            ...
            #clearHistoryBtn:active { background: rgba(255, 77, 77, 0.3); }
            ...
            #recentHistorySearch:focus {
                border-color: #00e0d0;
                box-shadow: 0 0 15px rgba(0, 224, 208, 0.3);
            }
            #recentHistorySearch::placeholder { color: #666; }
            ...
            #recentHistoryList::-webkit-scrollbar { display: none; }
            ...
            .history-play-icon {
                ...
                display: flex; align-items: center; justify-content: center;
                flex-shrink: 0;
                box-shadow: 0 2px 10px rgba(0, 224, 208, 0.5);
            }
    ...
    function injectOverlay() {
        if (document.getElementById('recentHistoryOverlay')) return;
        var overlay = document.createElement('div');
        overlay.id = 'recentHistoryOverlay';
        overlay.innerHTML = `
            <div id="recentHistoryHeader">
                <div id="recentHistoryBackBtn">
                    <svg viewBox="0 0 24 24" style="width:20px;height:20px;fill:#00e0d0;"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>
                </div>
                <div id="recentHistoryTitle">Recently <span>Played</span></div>
                <button id="clearHistoryBtn">Clear</button>
            </div>
            <input type="text" id="recentHistorySearch" placeholder="Search your history...">
            <div id="recentHistoryList"></div>
        `;
        document.body.appendChild(overlay);

        document.getElementById('recentHistoryBackBtn').onclick = closeHistory;
        document.getElementById('clearHistoryBtn').onclick = function() {
            if (confirm('Clear all listening history?')) {
                if (window.clearOfflineCache) window.clearOfflineCache();
                renderHistory();
            }
        };
        document.getElementById('recentHistorySearch').oninput = function(e) {
    ...
    function renderHistory(searchQuery) {
        ...
        var songs = [];
        if (window.getCachedSongs) songs = window.getCachedSongs();
        else {
            try {
        ...
        if (searchQuery) {
            ...
            songs = songs.filter(function(s) {
                return (s.title && s.title.toLowerCase().indexOf(q) >= 0) ||
                       (s.artist && s.artist.toLowerCase().indexOf(q) >= 0);
            });
        }
        ...
        if (songs.length === 0) {
            list.innerHTML = `
                <div class="history-empty">
                    ...
                    ${searchQuery ? 'No songs match your search.' : 'No songs played yet.<br>Start listening to see your history here!'}
                </div>
            `;
            return;
        }
        ...
        songs.forEach(function(s) {
            ...
            item.innerHTML = `
                <img src="${s.thumbnail || ''}" onerror="this.style.opacity='0.3'">
                <div class="history-info">
                    <div class="history-title">${(s.title || 'Unknown').replace(/</g, '&lt;')}</div>
                    <div class="history-artist">${(s.artist || 'Unknown').replace(/</g, '&lt;')}</div>
                    <div class="history-time">${formatTimeAgo(s.timestamp)}</div>
                </div>
    ...
    function openHistory() {
        injectOverlay();
        var overlay = document.getElementById('recentHistoryOverlay');
    ...
    function injectSeeAllBtn() {
        if (document.getElementById('seeAllRecentBtn')) return;
        var btn = document.createElement('div');
        btn.id = 'seeAllRecentBtn';
        btn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M13 3a9 9 0 00-9 9H1l4 4 4-4H6a7 7 0 117 7 6.98 6.98 0 01-4.9-2l-1.42 1.42A8.96 8.96 0 0013 21a9 9 0 100-18zm-1 5v5l4 2.7.75-1.23-3.25-2.02V8H12z"/></svg> See All History';
        document.body.appendChild(btn);
        btn.onclick = openHistory;
    }

    function updateSeeAllVisibility() {
        var btn = document.getElementById('seeAllRecentBtn');
        if (!btn) return;
        var songs = [];
        if (window.getCachedSongs) songs = window.getCachedSongs();
        if (songs.length > 0) btn.classList.add('show');
        else btn.classList.remove('show');
    }

    // Fix the home screen "Recently Played" — the missing section
    function fixHomeRecent() {
        var homeTab = document.getElementById('tab-home');
        if (!homeTab) return;
        var rp = document.getElementById('recentlyPlayed');
        if (!rp) {
            // Inject the recently played section if it's missing
            var container = document.createElement('div');
            container.innerHTML = `
                <div style="margin: 20px 0 10px 0;">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;padding:0 15px;">
                        <span style="font-size:18px;">🕒</span>
                        <span style="font-size:16px;font-weight:900;color:#fff;">Recently Played</span>
                    </div>
                    <div id="recentlyPlayed" style="display:flex;gap:12px;overflow-x:auto;padding:4px 15px 12px 15px;scrollbar-width:none;"></div>
                </div>
            `;
            // Insert at the top of the home tab
            if (homeTab.firstChild) homeTab.insertBefore(container, homeTab.firstChild);
            else homeTab.appendChild(container);
        }
    }

    // Render recent songs in the home section
    function renderHomeRecent() {
        ...
        if (songs.length === 0) {
            container.innerHTML = '<p style="color:#666;font-size:12px;padding:10px 0;">Play songs to see them here.</p>';
            return;
        }
        container.innerHTML = '';
        songs.slice(0, 8).forEach(function(s) {
            var card = document.createElement('div');
            card.style.cssText = 'flex:0 0 auto;width:130px;cursor:pointer;transition:transform 0.2s;';
            card.innerHTML = `
                <img src="${s.thumbnail || ''}" style="width:130px;height:130px;border-radius:12px;object-fit:cover;border:1px solid rgba(0,224,208,0.2);" onerror="this.style.opacity='0.3'">
                <div style="color:#fff;font-size:11px;font-weight:bold;margin-top:6px;line-height:1.3;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">${(s.title || '').replace(/</g,'&lt;')}</div>
                <div style="color:#888;font-size:9px;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${(s.artist || '').replace(/</g,'&lt;')}</div>
            `;
            card.onclick = function() {
                var mock = {
                    id: { videoId: s.id },
    ...
    window.openRecentHistory = openHistory;
    window.closeRecentHistory = closeHistory;
})();
// END RECENT HISTORY
